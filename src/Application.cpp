...
void Application::run() {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        for (int j = dynamicsWorld->getNumCollisionObjects() - 1; j >= 0; j--) {
            ...
            // gameobjects[j]->color = glm::vec3(trans.getOrigin().getX(), trans.getOrigin().getY(), trans.getOrigin().getZ());
            // ground->color = glm::vec3(1.0f, 0.0f, 0.0f);
            // trans.setRotation(btQuaternion(btVector3(0, 1, 0), xangle));
            
			// printf("world pos object %d = %f,%f,%f\n", j, float(trans.getOrigin().getX()), float(trans.getOrigin().getY()), float(trans.getOrigin().getZ()));
            glm::mat4 model = glm::mat4(1.0f);
            model = glm::scale(model, gameobjects[j]->scale);
